...
def checkpoint_assign(answered_questions) -> str:
    """
    This function records the players score at a certain index location and prints a password that allows them to return to that location with that score.
    ...
    -------
        password: str
            A string variable that refers to certain index locations within the person dictionaries.
    """
    ...
    print(Fore.GREEN + f"Checkpoint hit! Checkpoint is: {key}") #might want to move this to outside this function
    return key


def checkpoint_execute(key: str,checkpoint_dictionary: dict[str: list[tuple[float,float,float], tuple[int,int,int]]]) -> list[tuple[float,float,float], tuple[int,int,int]]:
    """
    ...
    """
    print("Where do you want to start at?")
    checkpoint_dictionary: dict[str: list[tuple[float,float,float], tuple[int,int,int]]] = {
        str : [(p1score, p2score, p3score),(p1Qanswered, p2Qanswered, p3Qanswered)]
    }
    dictionarykeys = list(checkpoint_dictionary.keys())

    for idx, key in enumerate(dictionarykeys):
        print(idx+1,key, sep=': ')
    checkp_choice = int(input("Pick the index of the checkpoint you want")) - 1
            ...
            if access == "y":
                ...
                try:
                    if password == "back" or "Back":
                        break
                    elif password not in dictionarykeys:
                        raise KeyError
                    break
                except KeyError:
                    print(Fore.YELLOW + "Please enter a valid password or type 'back'.")
            elif access == "n":
                break
        except KeyError:
...
def NextQuestion(questionNum, i = [0]):
    ...
    question: str = ""
    answers: list = []
    question = question + str(person1keys[questionNum]) #temporary
...
def friendsCnt(numpep, startvalue):
    ...
    if startloop == "oranges":
        print(Fore.YELLOW + "Support for multiple friends is currently unavalible, please come back later...\n...")
        print(Fore.YELLOW + "Friends is set to 1\n")
        numpep: int = 1
...
def nameSet(person_name):
    ...
    while True:
        try:
            ...
            print(Fore.YELLOW + "Name cannot be empty. Try again.")
    return person_name
...
def VictoryConditions(score):
    ...
    if 0.25 < score < 0.75: #Good ending!
        ...
        print(Fore.YELLOW + "how did you get here, this is not supposed to happen. Please report this to devs")
...
def StartGame(name, startval, numpeople):
    ...
    while True: #asks for input to do tutorial or not
        try: #this is for input validation
            ...
            print(Fore.YELLOW + "Invalid input, please type exactly \"1\" or \"2\"")
...
def MainGame(name):
    ...
    while VictoryCond == 0: #main loop for the game
        ...
        if questionnumber % 12 == 0: #checking the number of the question to spit out checkpoint
            checkp_score[checkpoint_assign(answered_questions)] = [tuple(MainScore), tuple(answered_questions)]  #giving checkp name and collecting score
        if numquestions > 1:
            print(Fore.MAGENTA + "──" + Fore.YELLOW + " ⋆⋅☆⋅⋆ " + Fore.MAGENTA + "──.·:*¨༺" + Fore.YELLOW + " ☾ " + Fore.MAGENTA + "༻¨*:·.──" + Fore.YELLOW + " ⋆⋅☆⋅⋆ " + Fore.MAGENTA + "──" + Fore.WHITE + f"\n\n{numquestions} questions remain\n")
        else:
            print(Fore.YELLOW + f"Final Question")
        sleep(1.5)
        print(Fore.BLUE + f"{curquestion}\n")
        sleep(2)
        ...
        while True:
            try: #this is for input validation
                ...
                print(Fore.YELLOW + "Invalid input, please type exactly one of the numbers shown")
            print()
        ...
        for i in range(len(MainScore)):
            ...
            if (MainScore[i] <= 0) or (MainScore[i] >= 1):
                ...
                print(Fore.RED + "Warning! Your friend is growing concerned!")
            elif MainScore[i] > 0.75:
                print(Fore.RED + "Warning! Hatred is seeping through your friend!")
            sleep(2)
            print("\n\n")
        questionnumber += 1
...
def EndingGame(victory, player_name):
    ...
    if victory == 1:
        print(Fore.GREEN + ending1_text)
        sleep(2) #1.2 conditions, flawless victory- all alive, all positive
    elif victory == 2:
        print(Fore.RED + ending2_text)
        sleep(2) #2 conditions- all eaten
    elif victory == 3:
        print(Fore.YELLOW + ending3_text)
        sleep(2) #3 conditions- all negative
    else:
        print(Fore.YELLOW + "unknown ending...\n...\n..")
        print(Fore.YELLOW + "bugged code srry")
...
def RetryGame():
    ...
    checkpoint_execute(checkpoint_dictionary)
...
def Menuscreen():
    ...
    while True:
        try: #this is for input validation
            ...
            menuinput: str = input("What do you want to do? Type \"Play\", \"Name\", \"Friends\", \"Load\", or \"Exit\"\n-> ").upper()
            inputcheck: list = ["PL", "NA", "FR", "EX", "LO"] #checking substrings for higher chance of getting right thing
            ...
            if "PL" in menuinput: #play
                if len(player_name) == 0:
                    if nonamechecknum == 0: #this is a funny easteregg
                        ...
                        print(Fore.YELLOW + "Please set a name first")
                        nonamechecknum -= 1
                else:
                    StartGame(player_name, startval, numofPeople)
                    break
                ...
                leaveY = input(Fore.YELLOW + "Do you really want to leave? Type \"Y\" if you do, or anything else for no\n-> " + Style.RESET_ALL)
                if leaveY == "Y":    
                    print("Thank you for playing!")
                    exit()
                ...
                print(Fore.RED + Style.BRIGHT + "how did you get here. bad !!@%$@#@#!!!$#@#@#@\n")
                raise KeyError
        except ValueError:
            print(Fore.YELLOW + "Invalid input, please type one of the five options\n")    
        except KeyError:
            exit()
...
if __name__ == '__main__':
    ...
    checkp_score: dict[str: list[tuple[float,float,float],tuple[int,int,int]]] = {} #scores for people, index of questions answered
