...
def checkpoint_assign(checkp_score: list[list[float]], MainScore: list[float]) -> list[list[float]]:
    """
    ...
    current_score = MainScore #collecting location of checkpoint and score
    placement: int
    placement = (sum(answered_questions) // 12) - 1 #index of question at specified checkpoint
    #call to update dictionary in here
    #if statements for each checkpoints specifying checkpoint name at specific answered_questions
    # this is for execute dict: {
    # key(checkpname): [location, score],
    #}
    checkp_score[key]: list[set] = [(MainScore) , (answered_questions)]
...
def NextQuestion(questionNum, i = [0]):
    ...
    return(question, answers, num_friend, i[0])
...
def MainGame(name):
    ...
    while VictoryCond == 0: #main loop for the game
        ...
        if questionnumber % 12 == 0: #checking the number of the question to spit out checkpoint
            checkp_score = checkpoint_assign(checkp_score, MainScore) #giving checkp name and collecting score
        if numquestions > 1:
...
if __name__ == '__main__':
    ...
    checkp_score: list[list[set]] = [[(_, _, _)]] #scores for people, index of questions answered
