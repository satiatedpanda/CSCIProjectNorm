...
def MainGame(name):
    ...
    while VictoryCond == 0: #main loop for the game
        ...
        anslenlist: list = []
        anslenstr: str = ""
        curquestion, curanswers, friend_number, function_call = NextQuestion(questionnumber)
        curquestion = curquestion.format(player_name = player_name) #formats the player name into the text
        if function_call % 4 == 0:
            checkp_score = checkpoint_assign(checkp_score, MainScore)
        if numquestions > 1:
